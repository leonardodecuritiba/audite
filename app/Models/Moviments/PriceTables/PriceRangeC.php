...
class PriceRangeC extends Model
{
    use SoftDeletes;
    use DateTimeTrait;
    use StringTrait;
	...
	{
		return route('conveyors.price-range.destroy-city',[$this->getAttribute('id'), 'C']);
	}
    ...
    {
        $value = '(Taxa Fixa) ' . DataHelper::getFloat2Currency($this->getAttribute('value_d'));
        $excess = '(+ Kg) ' . DataHelper::getFloat2Currency($this->getAttribute('excess_d'));
        return $value . '; ' .
            $excess;
    }
    ...
    {
        $value = '(Taxa Fixa) ' . DataHelper::getFloat2Currency($this->getAttribute('value_c'));
        $excess = '(+ Kg) ' . DataHelper::getFloat2Currency($this->getAttribute('excess_c'));
        return $value . '; ' .
            $excess;
    }
